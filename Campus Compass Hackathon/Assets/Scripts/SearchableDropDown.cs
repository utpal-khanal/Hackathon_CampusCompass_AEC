...
public class SearchableDropDown : MonoBehaviour
{
    ...
    [SerializeField] private List<Node> avlOptions = new List<Node>();
    [SerializeField] bool isSource = true;
    ...
    private void Awake()
    {
        Node.nodeSpawned += addNodeToList;
    ...
    public void AddItemToScrollRect(List<Node> options)
    {
        foreach (var option in options)
        {
            var buttObj = Instantiate(buttonsPrefab, content);
            buttObj.GetComponentInChildren<TMP_Text>().text = option.nodeName;
            buttObj.name = option.nodeName;
            buttObj.SetActive(true);
            var butt = buttObj.GetComponent<Button>();
            //butt.onClick.AddListener(delegate { OnItemSelected(buttObj); });
            butt.onClick.AddListener(delegate { OnItemSelected(option); });
    ...
    IEnumerator CheckIfValidInput(string arg)
    {
        ...
        for(int i =0; i< avlOptions.Count; i++) 
        {
            if (avlOptions[i].nodeName == arg)
            {
                // Message msg = new Message("Invalid Input!", "Please choose from dropdown",
                //                 this.gameObject, Message.ButtonType.OK);
                //
    ...
    private void OnInputvalueChange(string arg0)
    {
        for (int i = 0; i < avlOptions.Count; i++)
        {
            if (avlOptions[i].nodeName == arg0)
            {
                break;
            }
            if (i == avlOptions.Count - 1)
            {
                FilterDropdown(arg0);
    ...
    private void OnItemSelected(Node obj)
    {
        inputField.text = obj.nodeName;
        foreach (var button in initializedButtons)
            button.gameObject.SetActive(true);
        ...
        StopAllCoroutines();
        StartCoroutine(CheckIfValidInput(obj.nodeName));
